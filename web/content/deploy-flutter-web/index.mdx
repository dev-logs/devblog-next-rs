...
RUN curl \
  https://storage.googleapis.com/flutter_infra_release/releases/stable/linux/flutter_linux_3.13.8-stable.tar.xz \
  -o flutter-sdk.tar.xz
    ...
    steps:
      ...
      - name: Deploy
        run: |
          ...
          ssh -tt root@${{ secrets.SSH_HOST }} "docker pull ${{ env.IMAGE_NAME }}:latest && \
            docker rm -f simple_web && docker run -d --rm --name simple_web -p 3000:80 ${{ env.IMAGE_NAME }}:latest"
      - name: Cleanup
        run: rm -rf ~/.ssh
```
