...
use schema::devlog::devblog::rpc::post_interaction_response::InteractionResult;
use schema::devlog::devblog::rpc::post_service_server::PostService as PostGrpcService;
...
impl PostGrpcService for PostGrpcServer {
    ...
    async fn interact(
        &self,
        request: Request<PostInteractionRequest>
    ) -> Result<Response<PostInteractionResponse>, tonic::Status> {
        let user: &User = request.extensions().get::<User>().ok_or(Status::unauthenticated("You're not authorize"))?;
        ...
        match result {
            PostInteractionResult::Like(count) => Ok(Response::new(PostInteractionResponse {
                interaction_result: Some(InteractionResult::TotalLikeCount(count))
            })),
            PostInteractionResult::View(count) => Ok(Response::new(PostInteractionResponse {
                interaction_result: Some(InteractionResult::TotalViewCount(count))
            })),
            PostInteractionResult::Vote(count) => Ok(Response::new(PostInteractionResponse {
                interaction_result: Some(InteractionResult::TotalVoteCount(count))
            }))
        }
    }
}
